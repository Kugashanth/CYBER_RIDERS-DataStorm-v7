...
<<<<<<< HEAD
    ...
    """
    # 1. Identify feature columns and target variables
    feat_cols = [c for c in BASE_FEATURE_COLS if c in df.columns]
    ohe_cols  = [c for c in df.columns if c.startswith(("Outlet_Type_","Province_"))]
    all_feats = feat_cols + ohe_cols

    X = df[all_feats].fillna(0).values
    # Use maximum history as historical target baseline to extract unconstrained potential
    y_observed = df["hist_max_monthly"].values 

    # 2. Fit an structural Tobit baseline to evaluate unobserved capacity
    tobit_estimator = TobitRegressor(lower_limit=0.0)
    tobit_estimator.fit(X, y_observed)

    # 3. Predict the completely uncapped latent curve E[y*] 
    latent_target = tobit_estimator.predict_latent(X)

    # 4. Scale out seasonality parameters dynamically
    final_target = (latent_target 
